param(
  [string]$Configuration = "linux-x64",
  [string]$NodeVersion = ""
)
...
$nodeArchive = Join-Path $cache "node-v$NodeVersion-linux-x64.tar.xz"
$nodeUrl = "https://nodejs.org/dist/v$NodeVersion/node-v$NodeVersion-linux-x64.tar.xz"
$nodeExtractDir = Join-Path $cache "node-v$NodeVersion-linux-x64"
...
New-Item -ItemType Directory -Force -Path $appDir, $runtimeDir, $cache | Out-Null

if (!(Test-Path $nodeArchive)) {
  Write-Host "Downloading Node.js linux-x64 runtime:"
  Write-Host "  $nodeUrl"
